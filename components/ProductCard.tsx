...
export default function ProductCard({
  ...
  const inventoryItems = Array.isArray(product.inventory)
    ? product.inventory
    : [];
  ...
  const discountPercent = hasRealMrp
    ? Math.round(((mrpValue - finalPrice) / mrpValue) * 100)
    : 0;
  ...
  return (
    <div className="group flex h-full flex-col justify-between overflow-hidden rounded-2xl border border-[#ead8b8] bg-white transition-all duration-300 hover:-translate-y-0.5 hover:border-[#d7aa5b] hover:shadow-xl">
      <Link
      ...
      >
        <div className="relative aspect-square overflow-hidden bg-[#fffaf5]">
          ...
          {isOutOfStock && (
            <span className="absolute left-2.5 top-2.5 rounded-md bg-stone-800 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-white shadow-sm">
              Out of Stock
            </span>
          )}

          {discountPercent > 0 && !isOutOfStock && (
            <span className="absolute right-2.5 top-2.5 rounded-md bg-[#fff7e8] px-2 py-0.5 text-[10px] font-black text-[#741f23] shadow-sm">
              {discountPercent}% OFF
            </span>
          )}
        </div>
        ...
        <div className="space-y-2 p-3.5">
          ...
          <h3 className="line-clamp-2 min-h-[2.5rem] text-xs font-bold leading-5 text-stone-900 transition group-hover:text-[#741f23] sm:text-sm">
            {product.title}
          </h3>
              ...
              {availableSizes.length > 0 ? (
                <div className="flex flex-nowrap items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none">
                  {availableSizes.slice(0, 6).map(size => (
                    <span
                      key={size}
                      className="shrink-0 rounded-md border border-[#ead8b8] bg-[#fffaf5] px-2 py-1 text-[10px] font-bold text-stone-700"
                    >
                      {size}
                    </span>
                  ))}
                  ...
                  {availableSizes.length > 6 && (
                    <span className="shrink-0 text-[10px] font-bold text-[#741f23]">
                      +{availableSizes.length - 6}
                    </span>
                  )}
                </div>
      ...
      <div className="p-3.5 pt-0">
        {isOutOfStock ? (
          <button
            type="button"
            disabled
            className="flex min-h-11 w-full cursor-not-allowed items-center justify-center rounded-xl bg-stone-200 px-3 text-xs font-bold text-stone-500"
          >
            Out of Stock
          </button>
        ) : hasInventoryVariants ? (
          <Link
            href={`/product/${product.id}`}
            className="flex min-h-11 w-full items-center justify-center gap-1.5 rounded-xl bg-[#741f23] px-3 text-xs font-bold text-white shadow-sm transition hover:bg-[#5e171b] active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d7aa5b] focus-visible:ring-offset-2"
          >
