...
function publish(productId: string, summary: RatingSummary) {
  ...
  for (const listener of productListeners) listener(summary);
...
async function flushRatingBatch() {
  ...
  if (error) {
    for (const productId of productIds) publish(productId, { average: 0, count: 0 });
    return;
  }
  ...
  for (const productId of productIds) publish(productId, summaries.get(productId) || { average: 0, count: 0 });
}

function requestRating(productId: string, listener: (summary: RatingSummary) => void) {
  ...
  return () => {
    ...
    if (current && current.size === 0) listeners.delete(productId);
  };
}

export default function ProductRatingTag({ productId }: { productId: string }) {
  const [summary, setSummary] = useState<RatingSummary>(() => summaryCache.get(productId) || { average: 0, count: 0 });
  useEffect(() => requestRating(productId, setSummary), [productId]);

  return (
    <Link
      href={`/product/${productId}#reviews`}
      aria-label={summary.count ? `Open ${summary.average.toFixed(1)} star product reviews` : 'Write a product review'}
    ...
    >
      <Star size={11} fill={summary.count ? 'currentColor' : 'none'} className="shrink-0 text-[#d7aa5b] md:h-3 md:w-3" aria-hidden="true" />
      <span className="min-w-0 truncate">{summary.count ? `${summary.average.toFixed(1)} (${summary.count})` : 'Write a review'}</span>
    </Link>
  );
}
