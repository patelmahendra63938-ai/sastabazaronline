...
export async function calculateAuthoritativeOrderPricing(input: { db: SupabaseClient; pincode: string; paymentMethod: PricingPaymentMethod; cart: PricingCartItem[]; couponCode?: string; }): Promise<PricingBreakdown> {
  ...
  for (const item of input.cart) {
    ...
    const variantKey = item.pack_option_id ? `pack:${String(item.pack_option_id).trim()}` : `size:${String(item.size || 'Free Size').trim().toLowerCase()}`;
    const quantity = Number(item.quantity);
    if (!productId || !Number.isInteger(quantity) || quantity <= 0) throw new Error('Invalid retail cart quantity.');
    const key = `${productId}::${variantKey}`;
    const totalForProductVariant = (retailQuantityByProductVariant.get(key) || 0) + quantity;
    if (totalForProductVariant > MAX_RETAIL_QTY_PER_PRODUCT_SIZE) throw new Error('Retail orders allow a maximum of 5 units of the same product option. Please use Bulk Order / Contact Us for larger quantities.');
  ...
  for (const item of input.cart) {
    ...
    if (!Number.isInteger(exactPieceWeight) || exactPieceWeight <= 0) throw new Error(`Exact physical weight is missing for "${product.title}". Please update products.net_weight_grams in Admin.`);
    ...
    if (!Number.isFinite(basePackageLength) || basePackageLength <= 0 || !Number.isFinite(basePackageWidth) || basePackageWidth <= 0 || !Number.isFinite(basePackageHeight) || basePackageHeight <= 0) {
    ...
    const packOption = requestedPackOptionId
      ...
      : product.selling_mode === 'shared_pack' && requestedPackLabel
        ? packOptionsResult.data.find((row) => row.product_id === product.id && String(row.label || '').trim().toLowerCase() === requestedPackLabel)
        : null;
  ...
  const canApplyWelcome50 = hasNoCompetingCoupon && isWelcome50Active && discountedSubtotal >= (WELCOME50_OFFER.minimumOrder || 0) && productCampaignDiscount === 0;
  ...
  if (canApplyWelcome50) {
    ...
    const couponCents = Math.min(Math.round(WELCOME50_OFFER.discountValue * 100), subtotalBeforeCouponCents);
    ...
    verifiedItems.forEach((item, index) => {
      ...
      const proportionalCents = subtotalBeforeCouponCents > 0 ? Math.round((couponCents * lineCents) / subtotalBeforeCouponCents) : 0;
      const allocatedCents = isLastItem ? remainingCouponCents : Math.min(lineCents, remainingCouponCents, proportionalCents);
      const discountedLineCents = Math.max(0, lineCents - allocatedCents);
      item.line_total = discountedLineCents / 100;
      item.unit_price = item.quantity > 0 ? item.line_total / item.quantity : item.unit_price;
    ...
    discountedSubtotal = Math.round(verifiedItems.reduce((sum, item) => sum + item.line_total, 0) * 100) / 100;
    if (subtotalBeforeCoupon > 0) totalTaxAmount *= discountedSubtotal / subtotalBeforeCoupon;
    primaryOfferName = WELCOME50_OFFER.label;
  }

  const combinedShipment: ShippingPackageInput[] = [{ weight: actualWeightGrams, length: combinedPackageLength, width: combinedPackageWidth, height: combinedPackageHeight }];
  const shipping = await checkPincodeShippingRate(cleanPin, actualWeightGrams / 1000, discountedSubtotal, input.paymentMethod === 'COD' ? 'COD' : 'PREPAID', combinedShipment);
  if (!shipping.isServiceable) throw new Error(shipping.message || 'Delivery is currently unavailable for this PIN code.');
