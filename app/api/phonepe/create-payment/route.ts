...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      console.error(
        '[PHONEPE_CONFIG_ERROR] Supabase server configuration is missing.'
      );
    ...
    if (
      !pricing.serviceable ||
      !Number.isFinite(pricing.totalPayable) ||
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'A valid payable amount could not be calculated.',
        },
        { status: 400 }
      );
    }
    ...
    const expectedAmount =
      Math.round(pricing.totalPayable * 100) / 100;
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Unable to initialize secure payment session.',
        },
        { status: 500 }
      );
    }
    ...
    if (
      ...
      console.error(
        '[PHONEPE_CREATE_PAYMENT_INVALID_RESPONSE]',
        phonePeResponse
      );
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'PhonePe did not return a secure payment URL.',
        },
        { status: 502 }
      );
    }
