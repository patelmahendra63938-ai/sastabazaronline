...
export async function POST(request: Request) {
  ...
  try {
    ...
    const checkoutGst = parseCheckoutGstCookie(cookieStore.get(CHECKOUT_GST_COOKIE)?.value);
    ...
    const customerPhone = String(body.customer_phone || body.phone || '').replace(/\D/g, '').slice(-10);
    const address = String(body.address || '').trim();
    const city = String(body.city || '').trim();
    const state = String(body.state || '').trim();
    const pincode = String(body.pincode || '').trim();

    if (!customerName) return NextResponse.json({ success: false, error: 'Customer name is required.' }, { status: 400 });
    if (!customerEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customerEmail)) return NextResponse.json({ success: false, error: 'A valid customer email address is required.' }, { status: 400 });
    if (!/^\d{10}$/.test(customerPhone)) return NextResponse.json({ success: false, error: 'A valid 10-digit mobile number is required.' }, { status: 400 });
    if (!address || !city || !state) return NextResponse.json({ success: false, error: 'Complete delivery address, city and state are required.' }, { status: 400 });
    if (!/^\d{6}$/.test(pincode)) return NextResponse.json({ success: false, error: 'Please enter a valid 6-digit delivery PIN code.' }, { status: 400 });
    if (!Array.isArray(body.cart) || body.cart.length === 0) return NextResponse.json({ success: false, error: 'Your cart is empty.' }, { status: 400 });
    ...
    const cartPayload = body.cart.map((item) => ({
      ...
      pack_option_id: item.pack_option_id || undefined,
    }));

    const { error: sessionInsertError } = await db.from('phonepe_payment_sessions').insert({
    ...
    const phonePeRequest = StandardCheckoutPayRequest.builder().merchantOrderId(merchantOrderId).amount(amountPaise).redirectUrl(redirectUrl).build();

    let phonePeResponse;
    try {
      phonePeResponse = await client.pay(phonePeRequest);
    } catch (phonePeError) {
      await db.from('phonepe_payment_sessions').update({ status: 'CREATE_FAILED', phonepe_state: 'CREATE_FAILED', updated_at: new Date().toISOString() }).eq('merchant_order_id', merchantOrderId);
      throw phonePeError;
    }

    if (!phonePeResponse?.redirectUrl) {
      console.error('[PHONEPE_CREATE_PAYMENT_INVALID_RESPONSE]', phonePeResponse);
      await db.from('phonepe_payment_sessions').update({ status: 'CREATE_FAILED', phonepe_state: 'NO_REDIRECT_URL', updated_at: new Date().toISOString() }).eq('merchant_order_id', merchantOrderId);
      return NextResponse.json({ success: false, error: 'PhonePe did not return a secure payment URL.' }, { status: 502 });
    }

    return NextResponse.json({ success: true, merchantOrderId, redirectUrl: phonePeResponse.redirectUrl, amount: expectedAmount });
  } catch (error: unknown) {
    console.error('[PHONEPE_CREATE_PAYMENT_ERROR]', { merchantOrderId, error });
    return NextResponse.json({ success: false, error: error instanceof Error ? error.message : 'Unable to create PhonePe payment.' }, { status: 500 });
  }
}
