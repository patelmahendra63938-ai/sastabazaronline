            ...
            <div className="space-y-4">
              {orders.map((ord: any) => (
                ...
                >
                  ...
                  {(() => { const totals = resolveOrderTotals(ord); return <div className="mx-5 mb-4 grid grid-cols-2 gap-x-6 gap-y-1 rounded-xl border bg-gray-50 p-3 text-[11px] text-gray-600 sm:grid-cols-3"><span>Subtotal <b className="float-right text-gray-900">₹{totals.productSubtotal}</b></span>{totals.discountAmount > 0 && <span className="text-green-700">Discount <b className="float-right">-₹{totals.discountAmount}</b></span>}<span>Delivery <b className="float-right text-gray-900">₹{totals.shippingCharge}</b></span>{totals.isCod && <span>COD Charge <b className="float-right text-gray-900">₹{totals.codCharge}</b></span>}<span className="font-black text-indigo-950">Grand Total <b className="float-right">₹{totals.grandTotal}</b></span></div>; })()}
