      ...
      <main className="min-h-screen bg-[#fffaf5] flex flex-col justify-between font-sans">
        ...
        <div className="flex-1 max-w-xl mx-auto px-4 py-16 text-center space-y-4">
          ...
          <div className="pt-6 flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link href="/" className="bg-[#741f23] hover:bg-[#5e171b] text-white font-bold py-3.5 px-8 rounded-xl transition shadow-md text-xs uppercase tracking-wider">
              Continue Shopping
            </Link>
          </div>
        </div>
    ...
    return (
      <main className="min-h-screen bg-[#fffaf5] flex flex-col justify-between font-sans">
        ...
        <div className="flex-1 max-w-md mx-auto px-4 py-24 text-center space-y-4">
          ...
          <Link href="/" className="inline-block bg-[#741f23] hover:bg-[#5e171b] text-white font-bold py-3 px-6 rounded-xl transition shadow text-xs uppercase tracking-wider">
            Continue Shopping
          </Link>
        </div>
        <Footer />
    ...
    <main className="min-h-screen bg-[#fffaf5] flex flex-col justify-between font-sans pb-16">
      <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          <Link href="/cart" className="inline-flex items-center gap-2 text-xs font-bold text-[#741f23] mb-6 bg-[#fffdf9] px-4 py-2 rounded-xl border border-[#ead8b8] shadow-2xs transition hover:bg-[#fff7e8] hover:text-[#5e171b]">
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="bg-[#fffdf9] rounded-3xl border border-[#ead8b8] p-6 sm:p-8 shadow-xs space-y-6">
                <div className="flex items-center justify-between border-b border-[#ead8b8] pb-4">
                  ...
                  <button type="button" onClick={handleGetLocation} disabled={locating} className="flex items-center gap-1.5 text-xs font-bold bg-[#fff7e8] text-[#741f23] hover:bg-[#fff2dc] px-3 py-2 rounded-xl transition border border-[#ead8b8] cursor-pointer">
                ...
                <form onSubmit={handlePlaceOrder} id="checkout-form" className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="text" name="fullName" required value={formData.fullName} onChange={handleChange} placeholder="Recipient full name" className="w-full pl-9 pr-3 py-2.5 rounded-xl border text-xs font-medium focus:ring-2 focus:ring-[#d7aa5b] focus:outline-hidden bg-white border-[#ead8b8]" />
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="tel" name="phone" required inputMode="numeric" pattern="[0-9]{10}" maxLength={10} value={formData.phone} onChange={handlePhoneChange} placeholder="10-digit mobile number" className="w-full pl-9 pr-3 py-2.5 rounded-xl border text-xs font-mono font-medium focus:ring-2 focus:ring-[#d7aa5b] focus:outline-hidden bg-white border-[#ead8b8]" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input type="email" name="email" required value={formData.email} onChange={handleChange} placeholder="customer@example.com" className="w-full px-3 py-2.5 rounded-xl border text-xs font-medium focus:ring-2 focus:ring-[#d7aa5b] focus:outline-hidden bg-white border-[#ead8b8]" />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Delivery Address *</label>
                    <textarea name="address" required rows={3} value={formData.address} onChange={handleChange} placeholder="Flat/House No, Building, Street, Landmark" className="w-full px-3 py-2.5 rounded-xl border text-xs font-medium focus:ring-2 focus:ring-[#d7aa5b] focus:outline-hidden bg-white border-[#ead8b8]" />
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-gray-700 uppercase mb-1">City *</label>
                      <input type="text" name="city" required value={formData.city} onChange={handleChange} placeholder="Delivery city" className="w-full px-3 py-2.5 rounded-xl border text-xs font-medium focus:ring-2 focus:ring-[#d7aa5b] focus:outline-hidden bg-white border-[#ead8b8]" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 uppercase mb-1">State *</label>
                      <input type="text" name="state" required value={formData.state} onChange={handleChange} placeholder="Delivery state" className="w-full px-3 py-2.5 rounded-xl border text-xs font-medium focus:ring-2 focus:ring-[#d7aa5b] focus:outline-hidden bg-white border-[#ead8b8]" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button type="button" onClick={() => handleCheckPincode()} disabled={isCheckingPin || formData.pincode.length !== 6} className="px-4 py-2.5 bg-[#741f23] hover:bg-[#5e171b] text-white font-bold text-xs rounded-xl transition disabled:opacity-40 cursor-pointer shrink-0 flex items-center gap-1.5">
                        {isCheckingPin ? <><Loader2 size={13} className="animate-spin" /><span>Checking...</span></> : isPincodeVerified ? <><RefreshCw size={13} /><span>Recheck</span></> : <span>Check Delivery</span>}
                      </button>
                    </div>
                    <div className="mt-2">
                      {pinStatus === 'checking' && <p className="text-[11px] font-semibold text-gray-500 animate-pulse">Checking delivery availability with courier partner...</p>}
                      {pinStatus === 'available' && <div className="p-2.5 bg-green-50 border border-green-200 rounded-xl text-green-800 text-xs font-bold flex items-center gap-2"><Check size={15} className="text-green-600 shrink-0" /><span>{statusMessage}</span></div>}
                      {pinStatus === 'unavailable' && <div className="p-2.5 bg-red-50 border border-red-200 rounded-xl text-red-700 text-xs font-bold flex items-center gap-2"><XCircle size={15} className="text-red-600 shrink-0" /><span>{statusMessage}</span></div>}
                      {pinStatus === 'error' && <div className="p-2.5 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-xs font-bold flex items-center gap-2"><AlertCircle size={15} className="text-amber-600 shrink-0" /><span>{statusMessage}</span></div>}
                      {pinStatus === 'idle' && formData.pincode.length === 6 && !isPincodeVerified && <p className="text-[11px] font-bold text-[#b5843d]">Click &quot;Check Delivery&quot; to calculate shipping and enable order placement.</p>}
                    </div>
                  </div>

                  <div className="pt-4 border-t border-[#ead8b8]">
                    <h3 className="text-xs font-bold text-gray-800 uppercase tracking-wider mb-3">2. Payment Method</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <label onClick={() => handlePaymentMethodChange('COD')} className={`flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition ${formData.paymentMethod === 'COD' ? 'border-[#d7aa5b] bg-[#fff7e8]' : 'border-[#ead8b8] hover:border-[#d7aa5b]'}`}>
                        <input type="radio" name="paymentMethod" value="COD" checked={formData.paymentMethod === 'COD'} onChange={() => handlePaymentMethodChange('COD')} className="text-[#b5843d]" />
                        <div><span className="block text-xs font-bold text-gray-900">Cash on Delivery</span><span className="block text-[10px] text-gray-500">Pay cash at doorstep</span></div>
                      </label>
                      <label onClick={() => handlePaymentMethodChange('ONLINE')} className={`flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition ${formData.paymentMethod === 'ONLINE' ? 'border-[#d7aa5b] bg-[#fff7e8]' : 'border-[#ead8b8] hover:border-[#d7aa5b]'}`}>
                        <input type="radio" name="paymentMethod" value="ONLINE" checked={formData.paymentMethod === 'ONLINE'} onChange={() => handlePaymentMethodChange('ONLINE')} className="text-[#b5843d]" />
                        <div><span className="block text-xs font-bold text-gray-900">Pay Online Securely</span><span className="block text-[10px] text-gray-500">UPI, Cards & NetBanking via PhonePe</span></div>
                      </label>
                    </div>
                  </div>
                  ...
                  {formData.paymentMethod === 'ONLINE' && (
                    <div className="p-5 bg-[#fff7e8] border-2 border-[#ead8b8] rounded-2xl space-y-3">
                      <div className="flex items-center gap-2 text-[#741f23] font-bold text-sm"><ShieldCheck size={20} className="text-[#b5843d] shrink-0" /><span>Secure PhonePe Checkout</span></div>
                      <p className="text-[11px] leading-5 text-gray-600">You will be redirected to PhonePe&apos;s secure payment page to pay <span className="font-black text-[#741f23]">₹{grandTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>. After payment, you will return here automatically and your order will be created only after server verification.</p>
                      <div className="flex items-center gap-2 text-[11px] font-bold text-[#741f23] bg-white border border-[#ead8b8] rounded-xl px-3 py-2.5"><CreditCard size={15} className="text-[#b5843d]" /><span>UPI • Credit/Debit Cards • NetBanking</span></div>
                    </div>
                  )}
                </form>
              </div>
            ...
            <div className="lg:col-span-5">
              <div className="bg-[#fffdf9] rounded-3xl border border-[#ead8b8] p-6 shadow-sm sticky top-24 space-y-4">
                <div className="flex justify-between items-center border-b border-[#ead8b8] pb-3">
                  ...
                  <span className="text-[11px] font-mono font-bold bg-gray-100 text-gray-700 px-2.5 py-1 rounded-lg border border-gray-200">GSTIN: 24AKBPD1704F1Z1</span>
                </div>
                    ...
                    return (
                      <div key={`${itemId}-${item.size}`} className="py-3 flex items-center justify-between gap-3 text-xs">
                        <div className="flex items-center gap-3 overflow-hidden">
                          <Image src={resolveStorefrontImageSrc(item.image || (item.images ? item.images[0] : null))} alt={item.title} width={48} height={48} sizes="48px" className="w-12 h-12 object-cover rounded-xl border border-gray-200 shrink-0" />
                          <div className="min-w-0">
                            <h4 className="font-bold text-gray-900 truncate">{item.title}</h4>
                            <p className="text-[11px] text-gray-500 font-medium">Size: <span className="font-bold text-gray-800">{item.size || 'Free Size'}</span> • Qty: {itemQty} × ₹{itemPrice.toLocaleString('en-IN')}</p>
                            {item.applied_offer_label && <span className="text-[10px] font-bold text-green-700 flex items-center gap-1 mt-0.5"><Tag size={11} /> {item.applied_offer_label}</span>}
                          </div>
                        </div>
                        <div className="flex items-center gap-3 shrink-0">
                          <span className="font-black text-[#741f23]">₹{(itemPrice * itemQty).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                          <button type="button" onClick={() => setItemToRemove(item)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition cursor-pointer" title="Remove item"><Trash2 size={16} /></button>
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="border-t border-gray-100 pt-4 space-y-2.5 text-xs">
                  <div className="flex justify-between text-gray-600 font-medium">
                    ...
                    <span className="font-bold text-gray-900">₹{displayOriginalTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                  ...
                  {hasSaleDiscount && (
                    <div className="flex justify-between text-green-700 font-bold bg-green-50/80 px-2.5 py-1.5 rounded-xl border border-green-200">
                      <span>{displayOfferName || 'Current Offer'}</span>
                  ...
                  <div className="flex justify-between text-gray-700 font-semibold pt-1 border-t border-gray-100">
                    <span>Subtotal</span>
                    <span className="font-bold text-gray-900">₹{displaySubtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>

                  <div className="flex justify-between text-gray-500 font-medium"><span>Shipment Weight</span><span className="font-bold text-gray-800">{quote ? `${quote.actualWeightGrams} g` : 'Pending PIN check'}</span></div>

                  <div className="flex justify-between text-gray-500 font-medium">
                    <span>Delivery Charge</span>
                    <span className="font-bold text-gray-900">{isCheckingPin ? <span className="text-gray-400 font-normal">Calculating...</span> : !quote ? <span className="text-[#b5843d] font-bold">Pending PIN check</span> : `₹${quote.shippingCharge.toFixed(2)}`}</span>
                  </div>

                  {formData.paymentMethod === 'COD' && (
                    <div className="flex justify-between text-gray-500 font-medium"><span>COD Charge</span><span className="font-bold text-gray-900">{quote ? `₹${quote.codCharge.toFixed(2)}` : 'Pending PIN check'}</span></div>
                  )}

                  <div className="border-t border-[#ead8b8] pt-3 flex justify-between items-baseline text-sm font-black text-[#741f23]">
                    <span>Total Payable Amount</span>
                    <span className="text-base text-[#b5843d] font-black">₹{grandTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>

                <button type="submit" form="checkout-form" disabled={loading || cart.length === 0 || !quote || !isPincodeVerified || pinStatus !== 'available' || isCheckingPin} className="w-full bg-[#741f23] hover:bg-[#5e171b] text-white font-bold py-4 rounded-2xl transition shadow-md shadow-[#741f23]/20 flex items-center justify-center gap-2 text-xs uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer active:scale-98">
                  {loading ? <><Loader2 size={16} className="animate-spin" /><span>{formData.paymentMethod === 'ONLINE' ? 'Connecting to PhonePe...' : 'Verifying & Placing Order...'}</span></> : !isPincodeVerified ? <><Truck size={16} /><span>Verify PIN to Proceed</span></> : formData.paymentMethod === 'ONLINE' ? <><CreditCard size={16} /><span>Proceed to Secure Payment (₹{grandTotal.toLocaleString('en-IN')})</span></> : <><ShieldCheck size={16} /><span>Confirm & Place Order (₹{grandTotal.toLocaleString('en-IN')})</span></>}
                </button>

                <div className="bg-[#fff7e8] rounded-2xl p-3 border border-[#ead8b8] text-[11px] text-[#741f23] space-y-1">
                  <p className="flex items-center gap-1.5 font-bold"><FileText size={14} className="text-[#b5843d]" /> Tax Breakdown Info:</p>
                  <p className="text-gray-600 text-[10px]">All prices are inclusive of GST. Official tax invoice generated under <b>24AKBPD1704F1Z1</b>.</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {itemToRemove && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-[#fffdf9] rounded-2xl border border-[#ead8b8] max-w-sm w-full p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-gray-600">Are you sure you want to remove <span className="font-bold text-gray-900">{itemToRemove.title} ({itemToRemove.size || 'Free Size'})</span> from your checkout?</p>
            <div className="flex items-center justify-end gap-3 pt-2">
              <button type="button" onClick={() => setItemToRemove(null)} className="px-4 py-2 rounded-xl text-xs font-bold text-gray-600 hover:bg-gray-100 transition cursor-pointer">Cancel</button>
              <button type="button" onClick={() => handleRemoveItem(itemToRemove.id || itemToRemove.product_id, itemToRemove.size)} className="px-5 py-2 rounded-xl text-xs font-bold bg-red-600 hover:bg-red-700 text-white transition shadow cursor-pointer">Remove</button>
            </div>
          </div>
        </div>
      )}
