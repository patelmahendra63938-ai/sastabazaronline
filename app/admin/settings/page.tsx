...
export default async function AdminSettingsPage() {
  ...
  const integrationsConfigured = [
    Boolean(
      process.env.COURIER_API_KEY ||
        ...
        process.env.NIMBUSPOST_API_TOKEN
    ),
  ...
  const sections = [
    ...
    },
    {
      title: 'Checkout',
      description:
        'Payment method availability, minimum order, and customer-field rules.',
      status: 'Planned for Phase B',
      icon: CreditCard,
    },
    {
      title: 'Shipping',
      description: `${
        shipping.value.free_shipping_enabled
          ? 'Free shipping enabled'
          : 'Free shipping disabled'
      } · ${shipping.value.courier_markup_pct}% saved markup · version ${
        shipping.version || 1
      }`,
      status: 'Editable now',
      href: '/admin/shipping',
    ...
    },
    {
      ...
      description:
        'Control storefront filters and Amazon, Flipkart and Meesho visibility.',
      status: 'Editable now',
    ...
    },
    {
      title: 'Notifications',
      description:
        'Non-secret email and WhatsApp notification enablement.',
      status: 'Planned for Phase B',
      icon: Bell,
    },
    {
    ...
    <div className="mx-auto max-w-6xl space-y-8">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-xs leading-relaxed text-gray-500">
          Central access to operational settings and storefront visibility controls.
        </p>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-xs leading-relaxed text-indigo-950">
        ...
        />
        <p>
          Provider credentials remain environment-only. This settings area stores
          non-secret business rules and relies on existing staff/admin authorization.
        </p>
      </div>
        ...
        {sections.map(({ title, description, status, href, icon: Icon }) => {
          const content = (
            <>
              ...
              {href && (
                <span className="mt-5 inline-flex items-center gap-1 text-xs font-bold text-orange-600">
          ...
          return href ? (
            <Link
              key={title}
              href={href}
              className="rounded-2xl border border-gray-200 bg-white p-5 shadow-xs transition hover:-translate-y-0.5 hover:border-indigo-300 hover:shadow-md"
            >
              {content}
            </Link>
          ) : (
      ...
      <section className="space-y-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-xs sm:p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Existing specialized configuration remains available.
            </p>
          </div>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          <Link
            ...
            className="flex items-center justify-between rounded-xl border border-gray-200 p-4 text-xs font-bold text-gray-800 transition hover:border-indigo-300 hover:bg-indigo-50/40"
          >
          ...
          <Link
            ...
            className="flex items-center justify-between rounded-xl border border-gray-200 p-4 text-xs font-bold text-gray-800 transition hover:border-orange-300 hover:bg-orange-50/40"
          >
            <span className="flex items-center gap-2">
              <Tags size={16} className="text-orange-600" /> Discounts & Promotions
            </span>
            <ChevronRight size={14} />
          </Link>
        </div>
