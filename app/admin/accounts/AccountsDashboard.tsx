...
export default function AccountsDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === 'cod' && <Register title="COD Courier Settlements" icon={Truck} headers={['Reference', 'Partner', 'Date', 'COD collected', 'Deductions', 'Bank credit', 'Status']} rows={settlements.map(r=>{const c=calculateCodSettlement({codCollected:Number(r.cod_collected),courierCharge:Number(r.courier_charge),codCharge:Number(r.cod_charge),rtoCharge:Number(r.rto_charge),otherDeduction:Number(r.other_deduction)});return[r.settlement_reference,r.partner_name,new Date(r.settlement_date).toLocaleDateString('en-IN'),money(r.cod_collected),money(c.deductions),money(r.net_bank_credit),r.status]})} empty="No courier settlement imported yet." />}
      {tab === 'supplier' && <Register title="Supplier Tax Invoices" icon={ReceiptText} headers={['Supplier', 'Invoice', 'Date', 'Total', 'Paid', 'Outstanding']} rows={supplierInvoices.map(r=>[r.supplier_name,r.invoice_number,new Date(r.invoice_date).toLocaleDateString('en-IN'),money(r.total_amount),money(r.amount_paid),money(Number(r.total_amount)-Number(r.amount_paid))])} empty="No supplier invoice entered yet." />}
      {tab === 'credit' && <Register title="Return Credit Notes" icon={FileMinus2} headers={['Credit note', 'Date', 'Reason', 'Amount']} rows={creditNotes.map(r=>[r.credit_note_number,new Date(r.issue_date).toLocaleDateString('en-IN'),r.reason,money(r.total_amount)])} empty="No credit note issued yet." />}
