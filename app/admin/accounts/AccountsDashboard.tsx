...
type Order = { id: string; order_number?: string; created_at: string; customer_name?: string; grand_total?: number; shipping_charge?: number; cod_charge?: number; payment_method?: string; payment_status?: string; order_status?: string; order_items?: Array<{ line_total?: number; unit_price?: number; quantity?: number; gst_rate?: number }> };
...
export default function AccountsDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    const [ordersResult, supplierResult, settlementResult, creditResult] = await Promise.all([
      supabase.from('orders').select('id, order_number, created_at, customer_name, grand_total, shipping_charge, cod_charge, payment_method, payment_status, order_status, order_items(line_total,unit_price,quantity,gst_rate)').order('created_at', { ascending: false }),
    ...
    setOrdersError(ordersResult.error?.message || null);
  ...
  const grossSales = activeOrders.reduce((sum, order) => sum + Number(order.grand_total || 0), 0);
  const prepaidReceived = prepaidOrders.filter(order => order.payment_status === 'PAID').reduce((sum, order) => sum + Number(order.grand_total || 0), 0);
  const codExpected = codOrders.reduce((sum, order) => sum + Number(order.grand_total || 0), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      {ordersError && <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-xs font-semibold text-red-800">Order/GST data could not be loaded: {ordersError}</div>}
      ...
      {tab === 'prepaid' && <Register title="Prepaid Register" icon={Banknote} headers={['Order', 'Customer', 'Date', 'Amount', 'Status']} rows={prepaidOrders.map(o=>[o.order_number||o.id.slice(0,8), o.customer_name||'Customer', new Date(o.created_at).toLocaleDateString('en-IN'), money(Number(o.grand_total || 0)), o.payment_status||'PENDING'])} />}
